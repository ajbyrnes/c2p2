...
#include "lib/SZZlibCompressor.hpp"
...
int main() {
    ...
    std::cout << std::format("Data size: {} MB", dataSize * sizeof(float) / MB) << std::endl; 

    // Iterate over precision levels
    for (int precision{7}; precision > 0; --precision) {
        // Create compressor
        SZZlibCompressor compressor(precision, 9, SZ3::EB_REL, SZ3::ALGO_LORENZO_REG, SZ3::INTERP_ALGO_LINEAR, true);
